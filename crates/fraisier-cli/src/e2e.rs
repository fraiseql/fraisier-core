...
async fn deploy_commits_then_rolls_back_to_the_prior_release() {
    ...
    let first = SingleHostDeploy::builder("fraiseql", "production", HostId::new("localhost"))
        .context(deploy_ctx("v1", addr, &staging, &active))
        .artifact(Arc::new(ReleaseArtifact::new()))
        .migration(Arc::new(migration("v1", &log, &revfile)))
        .service(Arc::new(SystemdService::with_program(&systemctl)))
        .health(Arc::new(HttpHealth::new()))
        .build()
        .expect("build #1");

    let outcome = first.run(store.clone()).await.expect("run #1");
    ...
    let second = SingleHostDeploy::builder("fraiseql", "production", HostId::new("localhost"))
        .context(deploy_ctx("v2", addr, &staging, &active))
        .artifact(Arc::new(ReleaseArtifact::new()))
        .migration(Arc::new(migration("v2", &log, &revfile)))
        .service(Arc::new(SystemdService::with_program(&systemctl)))
        .health(Arc::new(HttpHealth::new()))
        .build()
        .expect("build #2");

    let outcome = second.run(store).await.expect("run #2");
    assert!(
